...
function UseFileStore() {
 ...
 const showToast = useCustomToast()
    ...
    const userUploadPost = async (file) => {
        ...
        const storage1Ref = ref(storage, `/images/${currentUser?.uid}/my-posts/${newPostId}`);
        const storage2Ref = ref(storage, `/images/community/${newPostId}`);
            ...
            uploadTask.on("state_changed",
                ...
                async () => {
                    try {
                        ...
                        const myPostRef = collection(db, `users/${currentUser?.uid}/my-posts`);
                        const newPost = {
                        ...
                        await addDoc(collection(db, "posts"), { ...newPost, uid: postRef.id });



                        showToast("Post upload", "success", 3, "Post was successfully uploaded to your profile!");
                    } catch (error) {
    ...
    const userDeletePost = async (uid, postId) => {
        try {
            ...
            showToast("Post delete confirmation","success",3,"Post was successfully deleted!")
        } catch (error) {
        ...
        const userSavePost = async (file) => {
            ...
            try {
                ...
                if (docSnapshot.exists()) {
                    ...
                    showToast("Post deleted from collection", "info", 3, "Post was successfully deleted from collection");
                } else {
                    const savedPost = {
                        imageUrl: imageUrl,
                        file_name: file_name,
